    ...
    if ( check_admin_referer( 'SocialReviewsJA', '_wpnonce' ) ) {
        try {
            ...
            add_settings_error(
                ...
                __( 'Settings updated successfully. Add shortcode [social-shortcode] to your page', 'social-reviews-ja' ),
                'updated'
            );
        } catch ( Exception $e ) {
            // Show error notice
...
<form method="post" action="">
    ...
    <input type="text" name="apikey" placeholder="API KEY" value="<?php if(get_option( 'social_reviews_api_key' ) !== null){ echo get_option( 'social_reviews_api_key' ); }?>">
    <input type="text" name="featureId" placeholder="Feature ID"  value="<?php if(get_option( 'social_reviews_dataID' ) !== null){ echo get_option( 'social_reviews_dataID' ); }?>">
